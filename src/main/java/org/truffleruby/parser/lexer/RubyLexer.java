...
public class RubyLexer implements MagicCommentHandler {
    ...
    /** Dedent the given node (a {@link StrParseNode} or a {@link ListParseNode} of {@link StrParseNode}) according to
     * {@link #heredoc_indent}. */
    public void heredoc_dedent(ParseNode root) {
        ...
        if (root instanceof StrParseNode) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                currentLine = line;
                // TODO(norswap, 28 Jan 2021): can it even be something else than a StrParseNode?
    ...
    /** Number of lines read in the source so far. TODO: is this always the lexb line? */
    private int line_count = 0;
    /** Index of the current source line (the first line has index 1). */
    ...
    /** Length of the current source line. TODO: This should always be equal to lex_pend, shouldn't it? */
    ...
    /** Returns a substring rope equivalent equivalent to the give rope (which contains a single line), dedented by the
     * given width. */
     ...
     * {@code indent} is true.
     * <p>
     * TODO: update this description when the function is fixed */
        ...
        if (indent) {
            for (int i = 0; i < lex_pend; i++) {
                if (!Character.isWhitespace(p(i + p))) {
                    // TODO(norswap): this is oh so wrong - but let's try to make a failing case
                    //  I don't even understand how anything with an indented end marker would parser correctly
                    //  - Also, this doesn't account for the fact that indent is a width, not a character length (so tab need special handling)
                    //  - Also, shouldn't the indent to be stripped before the end marker be fixed?
